...
export class GoogleMapComponent {
  ...
  constructor (
    ...
    this.options = mapService.MAP_OPTIONS;
      ...
      switch (this.editMode) {
        case EditModesEnum.MARKER:
          ...
          const pointOfInterest: PointOfInterest = {
            ...
            infoWindowOptions: this.mapService.InfoWindowOptions(this.point, new google.maps.Size(0, -37)),
        ...
        case EditModesEnum.POLYGON:
          ...
          const boundary: Boundary = {
            ...
            options: this.mapService.PolygonOptions(this.mapForm.get('iconColor')?.value),
            iconColor: this.mapForm.get('iconColor')?.value,
            infoWindowOptions: this.mapService.InfoWindowOptions(this.segments[Math.floor(this.segments.length / 2)], new google.maps.Size(0, 0))
          }
        ...
        case EditModesEnum.LINE:
          ...
          const path: Path = {
            ...
            description: this.mapForm.get('description')?.value,
            segments: this.segments,
            options: this.mapService.PolylineOptions(this.mapForm.get('iconColor')?.value),
            iconColor: this.mapForm.get('iconColor')?.value,
            infoWindowOptions: this.mapService.InfoWindowOptions(this.segments[Math.floor(this.segments.length / 2)], new google.maps.Size(0, 0))
          }
          paths.push(path);
  ...
  openInfoWindow(data: any, event?: any) {
    ...
    this.infoWindowOptions = data.infoWindowOptions ?? this.mapService.INFO_WINDOW_OPTIONS;
  ...
  initPointOfInterestForm() {
    ...
    this.subs.sink = combineLatest([
      ...
      this.pointOfInterestOptions = this.mapService.MarkerOptions({color: iconColor, icon: icon?.fontIcon, font: icon?.fontFamily});
    });
  }
  ...
  initPathForm() {
    ...
    this.subs.sink = this.mapForm.get('iconColor').valueChanges.pipe(startWith(null))
      .subscribe((iconColor) => {
      this.pathOptions = this.mapService.PolylineOptions(iconColor);
      this.linePointOptions = this.mapService.LinePointOptions({color: iconColor});
    });
  }
  ...
  initBoundaryForm() {
    ...
    this.subs.sink = this.mapForm.get('iconColor').valueChanges.pipe(startWith(null))
      .subscribe((iconColor) => {
      this.boundaryOptions = this.mapService.PolygonOptions(iconColor);
      this.linePointOptions = this.mapService.LinePointOptions({color: iconColor});
    });
  }
